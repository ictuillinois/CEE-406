...
- Chart theming is centralized in `src/components/react/chartTheme.ts` (useTheme hook, per-mode series palette, baseLayout). The palettes are validated (dataviz six-checks) per surface — light `#E87722/#0EA5E9/#10B981/#8B5CF6` on `#fff`, dark `#DC7014/#0C93CF/#0EA372/#8B5CF6` on `#162033`. Don't hardcode series colors in tools.
