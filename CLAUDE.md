...
Currently released: **tools** `gear3d`, `cross-section-studio`; **homeworks** none. Locked items render as a dimmed, dashed, non-interactive card with a lock chip reading "Coming soon", or "Releases week N" once `HOMEWORK_WEEK` / `TOOL_WEEK` carry the real semester calendar.
