...
- React tools are islands mounted with `client:only="react"` (Plotly is browser-only; do not SSR them). Each tool: `src/components/react/<tool>/App.tsx` + shared `src/components/react/tools.css` + a wrapper page in `src/pages/tools/<tool>/index.astro`, registered in `src/data/tools.ts`, which the tools index AND the landing page both render from. Never add a tool to only one of those surfaces — they used to keep separate arrays and the landing page silently drifted to advertising 2 tools while 18 were live. Nineteen tools are live: mr-fitter + cbr (HW2), mastercurve (HW2/8, Ch. 7 + App. A), lea + stress-explorer (HW3/4), esal-calculator + eswl (HW5/7), drainage (HW6), aashto (HW7/9), damage (HW8), pca + westergaard + joints (HW9), acr + lca (HW10), psi (HW1/10, Ch. 9), backcalc (Ch. 9/13), reliability (Ch. 10), cross-section-studio (figures, Ch. 1).
- **`cross-section-studio` is the one island that is not a React app.** It is a port of the standalone Three.js studio from `Johann-Cardenas.github.io/e-labs/cross-section-studio`, and the engine (`crosssection/studio.ts`) is a single `initStudio(root)` closure that owns the DOM imperatively and returns a disposer; `CrossSectionApp.tsx` supplies markup and calls it from one effect. Keep them apart — React must never be asked to re-render a WebGL scene, and the disposer must keep tearing down the rAF loop, the ResizeObserver and the document `keydown` handler or a remount leaves two render loops on one canvas. It also opts out of the `.cee-tool` two-column grid (via `.cee-tool.xs-tool`) while keeping its tokens, and it carries control help on `title` rather than `<Tip>`, because both side rails are `overflow: auto` and would clip an absolutely positioned popover. `three` is pinned to `0.160.1` — the version the upstream studio's import map pins — so the render is bit-for-bit the same.
