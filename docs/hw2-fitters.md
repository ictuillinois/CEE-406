...
Both axes use high-contrast labels, restrained major/minor grids, framed plots, and outlined blue markers. Continuous model curves use a deeper green. The generalized model is a surface in bulk and shear stress, not a single raw-modulus-versus-bulk curve. Its default fitted view divides each modulus by `(1 + tau_oct/pa)^k3` to expose the single continuous power curve `k1 pa (theta/pa)^k2`. The axis, equation, and hover text identify this transformation; original data, coefficients, residuals, and numerical predictions remain unchanged. A raw-response toggle shows three continuous confinement slices (low, middle, high) rather than numerous short curve segments. These slices are model projections where no corresponding test data exist. The bulk model already has one continuous power curve.
