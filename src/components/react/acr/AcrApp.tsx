...
import {
  parseRunwayCode, evaluate, PAVEMENT_TYPE, SUBGRADE, TYRE, EVALUATION,
...
export default function AcrApp() {
  ...
  const [tyre, setTyre] = useState('200');

  const theme = useTheme();
  const tyrePsi = num(tyre, 200);
  ...
  const results = useMemo(() => rows.map(r => {
    ...
    return {
      ...
      parsed,
      acr,
      verdict: parsed && Number.isFinite(acr) ? evaluate(parsed, acr, tyrePsi) : null,
    };
  }), [rows, tyrePsi]);
  ...
  const describe = (c: RunwayCode) =>
    ...
    `tyre ${c.tyre} (${TYRE[c.tyre].psi === null ? 'unlimited' : `≤ ${TYRE[c.tyre].psi} psi`}) · ${EVALUATION[c.method]}`;
      ...
      <aside className="cee-panel">
        ...
        <div className="cee-field">
          <label className="cee-field__label" htmlFor="acr-tp">
            <span>Tyre pressure<Tip text="Main gear tyre pressure. It is checked against the runway's tyre pressure code independently of the strength check — an aircraft can pass on strength and still be excluded on tyre pressure." /></span>
            <span className="cee-field__unit">psi</span>
          </label>
          <input id="acr-tp" className="cee-input" type="number" min="0" step="5" value={tyre}
        ...
        <p className="cee-hint">
          ...
          <code> PCR / type / subgrade / tyre / method</code>.
        </p>
      </aside>
      ...
      <div className="cee-results">
        <details className="cee-howto">
          ...
          <div className="cee-howto__body">
            <ol>
              <li><strong>Read the runway code</strong>: PCR number, R or F for rigid or flexible, the subgrade category A–D, the tyre pressure code W–Z, and how the rating was derived.</li>
              ...
              <li><strong>Check the tyre pressure separately.</strong> It is an independent gate; passing on strength does not help if the tyres are too hard for the surface.</li>
            </ol>
        ...
        <KpiStrip>
          <Kpi accent label="Runways usable" value={usable} unit={`of ${results.length}`}
            tip="Runways where the aircraft may operate without restriction: ACR within PCR and tyre pressure within the coded limit." />
          ...
          <Kpi label="Tyre pressure" value={fmt(tyrePsi, 0)} unit="psi"
            tip="Checked against each runway's tyre pressure code." />
        </KpiStrip>
          ...
          >
            {!r.parsed ? (
              <p className="cee-warn"><span className="cee-warn__icon">⚠️</span><span>
                ...
                <code> PCR/type/subgrade/tyre/method</code>, for example <code>700/R/C/Y/T</code>.
              </span></p>
        ...
        <p className="cee-note">
          ...
          different ACR on each runway. Tyre pressure codes cap the surface at W unlimited,
