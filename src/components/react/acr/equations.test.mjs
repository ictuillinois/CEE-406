...
import { parseRunwayCode, formatRunwayCode, evaluate, TYRE } from './equations.ts';

test('parses the HW10 runway ratings', () => {
  const a = parseRunwayCode('700/R/C/Y/T');
  assert.deepEqual(a, { pcr: 700, type: 'R', subgrade: 'C', tyre: 'Y', method: 'T' });
  assert.equal(formatRunwayCode(a), '700/R/C/Y/T');

  assert.deepEqual(parseRunwayCode('650/F/C/Y/T'),
    { pcr: 650, type: 'F', subgrade: 'C', tyre: 'Y', method: 'T' });
  assert.deepEqual(parseRunwayCode('600/F/B/X/T'),
    { pcr: 600, type: 'F', subgrade: 'B', tyre: 'X', method: 'T' });
...
test('rejects malformed ratings', () => {
  ...
  assert.equal(parseRunwayCode('700/R/C/Q/T'), null, 'bad tyre code');
...
test('tyre pressure is an independent gate', () => {
  // Code Y allows 181 psi; 200 psi fails even when the strength passes.
  const rw = parseRunwayCode('700/R/C/Y/T');
  const v = evaluate(rw, 600, 200);
  assert.equal(v.ok, false);
  assert.equal(v.overload, false, 'a tyre failure is not an overload case');
