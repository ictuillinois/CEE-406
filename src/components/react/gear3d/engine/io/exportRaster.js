...
export function inlineComputedColors(source, clone) {
    ...
    // PORT NOTE (CEE 406): upstream read this off document.documentElement,
    // where the app declared its tokens. Here they are declared on .g3-app —
    // the integration contract in docs/dashboard-visual-language.md §0.1 — so
    // documentElement no longer carries --g3-paper and this silently resolved
    // to '' on every export. `source` is the overlay SVG, which is inside
    // .g3-app and inherits the token, so it is the correct place to ask; it
    // would also have been the correct place upstream.
    const paper = cs.getPropertyValue('--g3-paper').trim() || '#ffffff';
