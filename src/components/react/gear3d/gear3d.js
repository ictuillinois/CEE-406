...
} from './engine/core/gearcode.js';
import { APP_REVISION } from './engine/core/version.js';
import { CAMERA_PRESETS, ENG_AXES } from './engine/core/coords.js';

import { MaterialLibrary, MATERIAL_SPECS } from './engine/scene/materials.js';
...
} from './engine/annotate/dimensions.js';
import { projectEng } from './engine/annotate/projection.js';
import { buildSnapPoints, nearestSnapPoint, dimensionFromSnaps } from './engine/annotate/snapping.js';

import { computePatches, patchTotals, patchOutlineAbsolute, DEFAULT_INFLATION_KPA } from './engine/contact/patch.js';
import { toCSV, toJSON as footprintJSON, toAbaqus } from './engine/contact/export.js';

import { paneAt } from './engine/views/quadview.js';
import {
...
} from './engine/views/isolation.js';

import {
    renderToCanvas, renderSupersampled, compositeOverlay, canvasToBlob, RESOLUTION_PRESETS
} from './engine/io/exportRaster.js';
import { exportGLTF, exportOBJ } from './engine/io/exportScene.js';
import { buildHybridSVG, buildHybridPDF } from './engine/io/exportVector.js';
import {
    serializeProject, parseProject, serializeUnit, download, readFileText,
    filenameFor, autosave, loadAutosave
} from './engine/io/project.js';
...
function renderChassisNotice() {
    ...
    box.hidden = false;
    box.innerHTML = iconHtml('drafting-compass') + '<span>'
...
function renderAssumptionNotice(u) {
    ...
    box.hidden = false;
    const lead = iconHtml('exclamation-triangle');
...
function setupGearCodePanel() {
    ...
    const read = () => {
        ...
        try {
            ...
            out.innerHTML =
                `<div class="g3-gc-ok">${iconHtml('check')} <span class="g3-mono">${esc(c.canonical)}</span></div>`
            ...
            out.innerHTML = `<div class="g3-gc-bad">${iconHtml('times')} Not a valid gear name</div>`
...
function buildCatalog() {
    ...
    for (const card of root.querySelectorAll('.g3-cat-card')) {
...
function catalogCard(code, section) {
    ...
    const loads = best && !schematic
        ? `<div class="g3-cat-loads">${iconHtml('cube')} Loads ${esc(model)}</div>`
        : '';
    ...
    return `<div class="g3-cat-card ${state}" ${best ? `data-unit="${esc(best.id)}"` : ''}`
        ...
        + `</div>`
        + (aircraft.length
            ? `<div class="g3-cat-ac">${iconHtml('plane')} ${esc(aircraft.join(', '))}</div>`
...
function setupDimensionPanel() {
    for (const cb of root.querySelectorAll('.g3-dimset')) {
        cb.addEventListener('change', () => {
            app.store.view.dimensionSets = Array.from(root.querySelectorAll('.g3-dimset'))
...
function renderOverrideStatus() {
    ...
    box.hidden = false;
    box.innerHTML = iconHtml('ruler-combined') + '<span>'
